...
class SchedulesEndpoint(BaseEndpoint):
    def __init__(
        ...
        :param bool query_schedule: True if querying details for an individual schedule, False otherwise.
    ...
    def base_schedule_url(self):
        return "{0}/api/{1}/schedules".format(self._connection.server, self._connection.api_version)

    @property
    def base_site_schedule_id_url(self):
        return "{0}/api/{1}/sites/{2}/schedules/{3}".format(
            self._connection.server, self._connection.api_version, self._connection.site_id, self._schedule_id
        )

    @property
    def base_schedule_id_url(self):
        return "{0}/{1}".format(self.base_schedule_url, self._schedule_id)
    ...
    def get_endpoint(self):
        ...
        if self._schedule_id:
            ...
            elif self._update_schedule or self._delete_schedule or self._query_schedule:
