...
class AddWorkbookPermissionsRequest(BaseRequest):
    ...
    def _get_capability_parameters_list(param_keys, param_values):
        ...
        for i, key in enumerate(param_keys):
            if param_values[i]:
                params_list.append({"name": key, "mode": param_values[i]})
        return params_list
    ...
    def modified_add_permissions_request(self):
        if any(self.optional_workbook_param_values):
            ...
            self._request_body["permissions"]["workbook"].update(
                self._get_parameters_dict(self.optional_workbook_param_keys, self.optional_workbook_param_values)
            )
        if self._user_capability_names:
            ...
            capability_dict.update(
                {
                    "capabilities": {
                        "capability": self._get_capability_parameters_list(
                            self._user_capability_names, self._user_capability_modes
                        )
                    }
                }
            )
            ...
            capability_dict.update(
                {
                    "capabilities": {
                        "capability": self._get_capability_parameters_list(
                            self._group_capability_names, self._group_capability_modes
                        )
                    }
                }
            )
